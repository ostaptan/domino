...
class DominosGame::Goat::Game < DominosGame::AbstractGame

  attr_accessor :market, :players, :battle_field, :first_bone

  def initialize
    super
    @battle_field = DominosGame::Goat::BattleField.new(self)
  end
  ...
  def load_dump(dump)
    ...
    @active = dump[:active]
    @first_bone = DominosGame::Goat::Bone.from_s dump[:first_bone]
    self
  end
