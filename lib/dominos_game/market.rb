...
class DominosGame::Market < Array
  include Enumerable
  ...
  def push_goat_bones(n1, n2)
    @bones << DominosGame::Goat::Bone.new(n1, n2) if n1 <= n2
  end

  def push_spider_bones(n1, n2)
    @bones << DominosGame::Spider::Bone.new(n1, n2) if n1 <= n2
  end
  ...
  def from_s s
    ...
    s.split('|').each do |bone_id|
      nums = bone_id.split('-')
      array << DominosGame::Goat::Bone.new(nums.first, nums.last)
    end
    replace array
  end
