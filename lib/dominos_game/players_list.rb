# -*- encoding : utf-8 -*-
require "./lib/dominos_game/player"

class DominosGame::PlayersList < Array
  ...
  def init(player_ids)
    push *player_ids.each_with_index.map{|player_id, index| player = DominosGame::Player.new(@game); player.init({player_id: player_id, position: index+1}); player}
    @finished = []
  end
  ...
  def load_dump(dump)
    replace dump[:players].map { |player_hash| DominosGame::Player.new(@game).load_dump(player_hash) }
